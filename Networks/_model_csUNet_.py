...
def conv2d_block(inputs, use_switch_norm=True, dropout=0.3, dropout_type="spatial", filters=16,
                 kernel_size=(3, 3), activation="relu", kernel_initializer="he_normal", padding="same"):
    # This function defines a convolutional block with optional switch normalization and dropout.
    ...
    c = Conv2D(filters, kernel_size, activation=activation, kernel_initializer=kernel_initializer, padding=padding, use_bias=not use_switch_norm)(inputs)
    
    if use_switch_norm:
        c = SwitchNormalization()(c)
    
    if dropout > 0.0:
        c = DO(dropout)(c)
    
    c = Conv2D(filters, kernel_size, activation=activation, kernel_initializer=kernel_initializer, padding=padding, use_bias=not use_switch_norm)(c)
...
def csUNet(input_shape, num_classes=1, activation="relu", use_switch_norm=True, upsample_mode="deconv",
    ...
    for l in range(num_layers):
        x = conv2d_block(inputs=x, filters=filters, use_switch_norm=use_switch_norm, dropout=dropout, dropout_type=dropout_type, activation=activation)
